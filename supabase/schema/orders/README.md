...
- **`supabase_store_scoping_orders.sql`** (#16, Clients #88) — de acá se extrajeron las policies de `orders`/`order_items`/`order_payments`. Lo que falta: los índices únicos `clients_unique_lot`/`clients_unique_otros_sin_desc` sobre la tabla `clients`, que este mismo archivo también scopeó por Store. (Encontrado al leer el archivo completo, no estaba en el AC.)
