...
| `refresh_product_affinity.sql` | Recalcula `product_affinity` desde cero. Se llama manualmente desde `/admin/informes`. Scoped por Store desde #21: `p_store_id` requerido, autorización vía `is_store_admin()`, co-ocurrencias filtradas por `orders.store_id`. El `TRUNCATE TABLE` (que vaciaba toda la tabla) pasa a `DELETE ... WHERE store_id = p_store_id OR store_id IS NULL`. |
