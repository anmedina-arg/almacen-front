...
| Archivo | Qué es |
|---|---|
| `product_affinity.sql` | Matriz de afinidad producto-a-producto, recalculada por `refresh_product_affinity()`. Lectura pública sin restricción — la API de recomendaciones del catálogo es pública. Desde #21 las filas nuevas llevan `store_id`; las viejas (previas a #21) quedan con `store_id NULL` hasta que cada Store dispare su primer refresh. |
| `category_affinity_rules.sql` | Reglas manuales de boost entre categorías, usadas como multiplicador dentro de `refresh_product_affinity()` — no como fuente directa de sugerencias, ver la nota en `get_recommendations.sql`. Policy de escritura scoped por Store desde #21 (`is_store_admin(store_id)`, antes chequeaba rol global). Sin UI de CRUD todavía — las reglas existentes se gestionan a mano en el SQL Editor. `store_id` es NOT NULL desde #22 — las 2 reglas que hay hoy en producción ya tenían `store_id` seteado (market-del-cevil), la suposición de "todas con store_id NULL" de #21 no se verificó contra datos reales en su momento. El puente de negocio en `refresh_product_affinity.sql` (reglas sin `store_id` aplicando a todas las Stores) sigue existiendo como diseño para el día que haya reglas realmente globales — no está en uso hoy porque no hay ninguna fila así. |
...
| `refresh_product_affinity.sql` | Recalcula `product_affinity` desde cero. Se llama manualmente desde `/admin/informes`. Scoped por Store desde #21: `p_store_id` requerido, autorización vía `is_store_admin()`, co-ocurrencias filtradas por `orders.store_id`. El `TRUNCATE TABLE` (que vaciaba toda la tabla, de todas las Stores) pasa a un `DELETE` acotado a las filas propias de esta Store más las filas legacy (`store_id IS NULL`) de los pares de producto que se están recalculando — nunca "cualquier fila NULL de la tabla" (eso pisaría datos de una Store que todavía no corrió su propio refresh). |
