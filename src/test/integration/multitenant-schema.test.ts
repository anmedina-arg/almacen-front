...
describe.skipIf(!hasTestCredentials)('schema expand multi-tenant (#10)', () => {
  const supabase = createClient(url!, key!);

  it('la tabla stores existe y es consultable', async () => {
    const { error } = await supabase.from('stores').select('id').limit(1);
    expect(error).toBeNull();
  });

  it.each(TABLES_WITH_STORE_ID)('%s tiene la columna store_id', async (table) => {
