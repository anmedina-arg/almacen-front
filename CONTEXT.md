...
Quién es dueño de una porción de lógica de negocio y sus invariantes — eje de arquitectura, independiente de Feature flag (comercial) y de superficie de UI (¿tiene página propia?). Los límites de dominio de la capa de servicios están registrados en [ADR-0013](./docs/adr/0013-service-layer-domain-boundaries.md).
_Avoid_: Usar "feature" o "módulo" como sinónimo de Dominio — un Dominio puede no tener flag propia (Producto Surtido) o estar anidado dentro de otro (Combos dentro de Products).
